...
import { humanizePointEditDate } from '../utils.js';

function createCityElement (cities) {
  return cities.map((city) => (`<option value="${city}"></option>`)).join(' ');
}

function createEventEditHeaderTemplate(point) {
  const { type, destination, dateFrom, dateTo, basePrice } = point;

  const destinationPoint = getDestinationById(destination);
  const dateStart = humanizePointEditDate(dateFrom);
  const dateEnd = humanizePointEditDate(dateTo);
...
export default class EventEditHeaderView {
  ...
  getTemplate() {
    return createEventEditHeaderTemplate(this.point);
  }
